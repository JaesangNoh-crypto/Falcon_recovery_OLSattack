...
def ffsampling_fft2(t, T, sigmin, randombytes):
    """Compute the ffsampling of t, using T as auxilary information.
    ...
    if (n > 1):
        l10, T0, T1 = T
        z1, z1_bar = ffsampling_fft2(split_fft(t[1]), T1, sigmin, randombytes)
        z[1] = merge_fft(z1)
        z_bar[1] = merge_fft(z1_bar)
        t0b = add_fft(t[0], mul_fft(sub_fft(t[1], z[1]), l10))
        z2, z0_bar = ffsampling_fft2(split_fft(t0b),  T0, sigmin, randombytes)
        z[0] = merge_fft(z2)
        z_bar[0] = merge_fft(z0_bar)
        return z, z_bar
    elif (n == 1):
        z[0] = [samplerz(t[0][0].real, T[0], sigmin, randombytes)]
        z_bar[0] = [int(floor(t[0][0].real))]
        z[1] = [samplerz(t[1][0].real, T[0], sigmin, randombytes)]
        z_bar[1] = [int(floor(t[1][0].real))]
        return z, z_bar
