...
from fft import fft, ifft, sub, add, neg, add_fft, mul_fft
from ntt import sub_zq, mul_zq, div_zq
from ffsampling import gram, ffldl_fft, ffsampling_fft, ffsampling_fft2
...
class PublicKey:
    ...
    def __init__(self, sk):
        ...
        self.random_point = sk.random_point
...
class SecretKey:
    ...
    secret key opera    tions (and also public key operations) in Falcon.
    ...
    def sample_preimage(self, point, seed=None):
        ...
        if seed is None:
            ...
            z_fft, z_bar_fft = ffsampling_fft2(t_fft, self.T_fft, self.sigmin, urandom)
        else:
            # If a seed is defined, initialize a ChaCha20 PRG
            # that is used to generate pseudo-randomness.
            chacha_prng = ChaCha20(seed)
            z_fft, z_bar_fft = ffsampling_fft2(t_fft, self.T_fft, self.sigmin, chacha_prng.randombytes)


        ## find t_hat and z
        z_bar0 = [int(round(elt)) for elt in ifft(z_bar_fft[0])]
        z_bar1 = [int(round(elt)) for elt in ifft(z_bar_fft[1])]
        ...
        z_bar = [sub(z_bar0,z0), sub(z_bar1,z1)]

        x_poly = [1/2]*len(z0)

        x = add(x_poly,z_bar[0]) + add(x_poly,z_bar[1])
